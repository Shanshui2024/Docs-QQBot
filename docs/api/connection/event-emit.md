...
| id | 事件id | 
| op | `opcode` ，参考连接维护 | 
|  s | 下行消息序列号 用于标识消息的**唯一性**；客户端再发送心跳时需携带客户端收到的最新的s |
|  t | 事件类型。主要用在op为 `0` 的时候 |
|  d | 事件内容，不同事件类型的事件内容格式**不同**，请注意识别。主要用在op为 `0` 的时候 |


### OpCode 含义

:::warning[注意]
该内容源于 [OpCode含义 - 事件订阅与通知 | QQ机器人官方文档](https://bot.q.qq.com/wiki/develop/api-v2/dev-prepare/interface-framework/event-emit.html#opcode-%E5%90%AB%E4%B9%89) ，未经验证。
:::
|**OPCODE**| **名称**      | 接入方式           | **客户端行为**| **描述**                                  |
|---------|-----------------|-------------------|---------------|------------------------------------------|
...
def handle_validation(rw, r, bot_secret):
    try:
        http_body = json.loads(r.body)
        payload_data = http_body.get('data')
        if not payload_data:
            logging.error("Missing 'data' field")
            return
        validation_payload = json.loads(payload_data)
        ...
        while len(seed) < 32:
            seed = seed * 2
        seed = seed[:32]
        private_key = Ed25519PrivateKey.from_private_bytes(seed.encode('utf-8'))
        # 签名
        msg = event_ts + plain_token
        signature = private_key.sign(msg.encode('utf-8')).hex()
        # 响应
        response = json.dumps({
        ...
        logging.error(f"Handle validation failed: {e}")
```
  </TabItem>
</Tabs>
...
然后进行  `websocket` 长连接建立，一旦连接成功，就会返回 [OpCode 10 Hello](event-emit#opcode-含义) 消息。这个消息主要的内容是心跳周期，单位毫秒(milliseconds)，如下：
```json
{
  "op": 10,
  "d": {
...
心跳发送成功之后会收到 [OpCode 11 Heartbeat ACK](event-emit#opcode-含义) 消息， `payload` 如下：
...
有很多原因可能会导致  `websocket` 长连接断开，断开之后短时间内重连会补发中间遗漏的事件，以保障业务逻辑的正确性。断开重连 gateway 后不需要发送重新登录 [Opcode 2 Identify](event-emit#opcode-含义)请求。在连接到  `Gateway` 之后，需要发送 [Opcode 6 Resume](event-emit#opcode-含义)消息， `payload` 如下：
```json
{
  "op": 6,
  "d": {
